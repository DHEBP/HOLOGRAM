...
(function() {
  ...
  // ── File drop guard ───────────────────────────────────────────────────────
  // Kept byte-identical to server_manager.go and Browser.svelte; TestFileDropBridgeParity
  // fails if they drift. A locally-served app must be able to receive a dropped file, or
  // drag-and-drop can only ever be tested against something already published on chain.
  //
