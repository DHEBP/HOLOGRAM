...
### Fixed
- Smart contract writes that the chain cannot apply are refused before broadcast, on the Explorer variable editor, the Explorer/Studio function caller, and INDEX updates. Over the storage-gas ceiling a write is still mined and still charged while the contract stores nothing — HOLOGRAM now checks the cost first and says how much to cut. The check is against the chain's per-call ceiling only; a write below it can still be discarded when the fee attached does not cover what the contract stores, which is decided in the wallet layer and is not something this guard can see. When the cost cannot be measured the write proceeds as before rather than being blocked on a failed measurement; dApp calls arriving over XSWD are not covered yet.
