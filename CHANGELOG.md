...
- **A contract deployed by a dApp is signed by the wallet that dApp is connected to.** A dApp can deploy by sending `transfer` with an `sc` field, the way Engram and the TELA CLI accept it. HOLOGRAM handed that to the generic install path, which decided the signer again on its own: in simulator mode it took wallet #0 regardless of which wallet was open, while `GetAddress`, `GetBalance` and `scinvoke` on the same connection all answered for the open wallet. A contract that records its deployer — `STORE("owner", SIGNER())`, the ordinary pattern — therefore recorded a wallet the dApp had never connected to, and refused its own creator on the next call. The deployment is now signed by the wallet the call already resolved. The primary-wallet rule it replaced was justified by funding, but on the simulator every registered wallet is credited the same amount and none of the pre-seeded wallets is premined, so wallet #0 held no advantage; off simulator the rule never applied. The Studio deploy screen and the simulator developer deploy now resolve their signer the same way every TELA deployment already did: the open wallet, with the primary as a fallback when none is open.
- **A deployment arriving from a dApp no longer freezes the wallet off simulator.** The same handoff called `GetWallet()` from inside a function that holds the wallet manager's write lock for its whole body. `sync.RWMutex` is not reentrant, so that read lock could never be acquired by the goroutine already holding the write lock: the call never returned, the deferred unlock never ran, and every later wallet operation blocked behind it. The install core no longer touches the wallet manager at all — it signs with the wallet it is given.

### Changed
- **The approval prompt names the wallet that will sign, and says when a deployment is what is being approved.** A `transfer` carrying `sc` has no destination, no SCID and no entrypoint, so it reached the prompt as an empty request while a contract was about to be deployed under a signature the user could not see. The prompt now shows the signing wallet for every signing request, and announces a contract deployment with the size of the code.
